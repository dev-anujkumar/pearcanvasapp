...
ajax.send = function (url, callback, method, data, contentType, sessionCookieName, tokenId, sync) {
    console.log("ajax.send props: " + url, callback, method, data, contentType, sessionCookieName, tokenId, sync);
    var x = ajax.x();
    console.log("x from ajax.send: " + x);
...
ajax.authenticate = function (url, callback, data, headers, sync) {
    console.log("ajax.authenticate props: " + url, callback, data, headers, sync);
    var x = ajax.x();
...
ajax.get = function (url, callback, contentType, sessionCookieName, tokenId, sync) {
    console.log("ajax.get props: " + url, callback, contentType, sessionCookieName, tokenId, sync);
...
ajax.post = function (url, data, callback, contentType, sync) {
    console.log("ajax.post props: " + url, data, callback, contentType, sync);
    var query = [];
    for (var key in data) {
...
function openamConfig(options) {
    ...
    this.openAMBaseURL = function () {
        console.log("openAMBaseURL: " + options.baseurl);
        return options.baseurl;
    };

    this.realm = function () {
        if (options.realm) {
            console.log("options.realm: " + options.realm);
    ...
    ajax.get(serverinfo_url,
        function (responseText) {
            ...
            console.log("response_sir: " + response_sir);
        },
        'application/json');
    ...
    this.ssotoken = function () {
        console.log("ssotoken_: " + ssotoken_);
        return ssotoken_;
    };

    this.XUILoginURL = function () {
        console.log("options.baseurl from XUILoginURL: " + options.baseurl);
        return options.baseurl + this.XUILoginURI();
    };

    this.loginURL = function () {
        console.log("options.baseurl from loginURL: " + options.baseurl);
...
openamConfig.prototype.authenticateWithModernOpenAM = function (options) {
    if (options) {
        ...
        console.log("authenticateWithModernOpenAM headers: " + headers);
 ...
 */
openamConfig.prototype.authenticateSimple = function (options) {
    console.log("authenticateSimple")
    var gotoURL = options.gotoURL || getMyURL();
    var gotoOnFail = options.gotoOnFail || getMyURL();
    try {
...
const redirectParent = (urlToBeRedirected) => {
    if (window.parent.location.pathname === "/") { // Remove this IF block once new Dashboard comes up //
