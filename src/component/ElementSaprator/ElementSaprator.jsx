...
export function ElementSaprator(props) {
    ...
    const renderPasteButton = (separatorProps, type) => {
        ...
        if(config.isPopupSlate && props.elementSelection?.element?.type === ELEMENT_ASIDE){
    ...
    const renderWordPasteButton = (parentElementType, { firstOne, index, userRole, onClickCapture }) => {
        ...
        const parentContainerForShowHide = ["groupedcontent", ELEMENT_ASIDE]
        ...
        if (inContainer.includes(parentElementType) || config.isPopupSlate || !isPasteFromWordBtn || (asideData?.type ===ELEMENT_ASIDE && parentContainer.includes(asideData?.parent?.type)) || (asideData?.type === SHOW_HIDE && parentContainerForShowHide.includes(asideData?.grandParent?.asideData?.type)) || (parentUrn?.subtype === TAB) || (asideData?.type === MULTI_COLUMN && asideData?.subtype === TAB)) {
            return null;
        }
    ...
    */
    if([ELEMENT_ASIDE,'groupedcontent'].indexOf(props?.asideData?.type) > -1 && props?.elementSelection?.containsBlockList) {
        pasteRender = false;
    }
    /* @hideSplitSlateIcon@ hide split slate icon in following list of elements */
    const hideSplitSlateIcon = !([ELEMENT_ASIDE, 'citations', 'poetry', 'group','showhide'].includes(elementType));
