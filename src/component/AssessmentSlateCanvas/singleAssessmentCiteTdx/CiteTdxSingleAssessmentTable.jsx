            ...
            <div>
                ...
                <CiteLoader isLoading={this.props.isLoading} citeErrorFlag={this.props.citeErrorFlag} />
                        <table className='assessment-table-class single-assessment'>
                            ...
                            {(isLoading == false) && (assessmenterrFlag == false) && this.props.singleAssessmentData && this.props.singleAssessmentData.data &&
                             this.props.singleAssessmentData.data.length > 0 &&
                            <tbody>
                                            ...
                                            <tr className ={this.state.activeRow && this.state.activeRow== item.versionUrn ? 'selected':''}>
                                                <td className="td-class">
                                                    <input type="radio" className="radio-button" name="assessment-radio" value={item.versionUrn}
                                                     onClick={() => this.addAssessment(item)} checked={this.props.currentSingleAssessmentSelected.versionUrn=== item.versionUrn} />
                                                ...
                                                <td><span className="modifiedby-date" title={item.dateModified ? moment(item.dateModified).format('DD MMM YYYY, hh:mmA') : ""}>
                                                    {item.dateModified ? moment(item.dateModified).format('DD MMM YYYY, hh:mmA') : "NA"}</span></td>
                        ...
                        {(singleAssessmentData && singleAssessmentData.data && singleAssessmentData.data.length == 0) && (isLoading == false) && (assessmenterrFlag == false) &&
