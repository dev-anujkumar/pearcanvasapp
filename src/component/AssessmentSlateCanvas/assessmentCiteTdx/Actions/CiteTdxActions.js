...
export const getCiteTdxData = (assessmentType, assessmentTitle, filterUUID, pageNo=1) => (dispatch) => {
    ...
    let url = `${config.ASSESSMENT_ENDPOINT}assessments/v3/search?taxonomicTypes=${assessmentType === FULL_ASSESSMENT_CITE ? `CITE` : assessmentType === FULL_ASSESSMENT_CITE? `TDX` :'MMI'}&status=approved&name=${searchTitle}&page=${pageNo}&pageSize=${pageSize}`;
...
export const filterCiteTdxData = (assessmentType, assessmentTitle, filterUUID) => (dispatch) => {
    ...
    let url = `${config.ASSESSMENT_ENDPOINT}assessments/v2/urn:pearson:work:${filterUUID}`;
