...
import { POD_DEFAULT_VALUE, elementAuthoredText, formattedTitleText, schemaUrl } from '../../constants/Element_Constants'
...
export const getMetaDataFieldForPopup = ({ popupdata: _popupdata }, _previousElementData) => {
    let hasFormattedTitle = _popupdata.hasOwnProperty(formattedTitleText),
    ...
    switch (type){
        ...
        case elementTypeConstant.POETRY_STANZA:
            ...
            dataToReturn = {
                ...
                inputType : parentElement && (parentElement.type === "popup" || parentElement.type === "citations" || parentElement.type === "poetry" && previousElementData.type === elementAuthoredText) ? "AUTHORED_TEXT" : inputElementType,
                inputSubType : parentElement && (parentElement.type == "popup" || parentElement.type === "poetry") ? "NA" : inputElementSubType
            }

            if(type === elementAuthoredText){
            ...
            if(parentElement && parentElement.type === "popup"){
                ...
                if(parentElement.contents && parentElement.contents[formattedTitleText] && parentElement.contents[formattedTitleText]["id"] === previousElementData.id){
                ...
                const poetryElementTypes = ['poetry', 'stanza', elementAuthoredText];
                ...
                if (elementType && poetryElementTypes.includes(elementType) && parentElement?.interactivedata) {
                    parentElement?.interactivedata?.[showHideType].forEach(poetryElement => {
                        if(poetryElement?.type === 'poetry') {
                            if(poetryElement?.contents?.[formattedTitleText]?.id === previousElementData.id) {
                                dataToReturn["metaDataField"] = "formattedTitle";
