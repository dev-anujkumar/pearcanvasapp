...
export const handleAutonumberingOnCreate = (type, createdElementData) => async (dispatch, getState) => {
    ...
    switch (type) {
        case 'IMAGE':
        case 'VIDEO':
            slateFigures = await getAutoNumberedElementsOnSlate(getState().appStore.slateLevelData[config?.slateManifestURN], { dispatch });
            break;
        case 'CONTAINER':
        case 'WORKED_EXAMPLE':
            slateFigures = getAsideElementsWrtKey(bodyMatter, 'element-aside');
            break;
        default:
            slateFigures = [];
    }
