...
class HtmlDiff {
    ...
    processDeleteOperation(opp, cssClass) {
        ...
        let assetId = this.processAssetIdOld(opp);
        this.insertTag("del", cssClass, text, assetId);
    }

    processAssetIdOld(opp) {
        if (this.oldWords[opp.startInOld - 1] && this.oldWords[opp.startInOld - 1].includes("asset-id")) {
            return this.oldWords[opp.startInOld - 1].split("asset-id")[1].split(" ")[0].replace(/"/g, "").replace("=", "")
        }
        else if (this.oldWords[opp.startInOld].includes("asset-id")) {
            return this.oldWords[opp.startInOld].split("asset-id")[1].split(" ")[0].replace(/"/g, "").replace("=", "")
        }
        else {
            return;
        }
    ...
    insertTag(tag, cssClass, words, assetId) {
        while (words.length) {
            ...
            if (nonTags.length !== 0) {
                let text = Utils.wrapText(nonTags.join(''), tag, cssClass, assetId);
                ...
                if (specialCaseOpeningTagRegex.test(words[0])) {
                    ...
                    if (openingTag != null && openingTag.match(specialCaseOpeningTagRegex)) {
                        let matchedTag = openingTag.match(specialCaseOpeningTagRegex);
                        openingTag = '<' + matchedTag[0].replace(/(<|>| )/g, '') + '>';
                    }
...
HtmlDiff.execute = function (oldText, newText) {
    ...
    if (newText.includes('inline-code-attacher')) {
        newText = TCMUtils.replaceAll(newText,textToReplace,'');
    }
